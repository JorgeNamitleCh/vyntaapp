...
const mb = StyleSheet.create({
  bg:   { height: 6, backgroundColor: C.border, borderRadius: 3, marginTop: 8 },
  fill: { height: 6, backgroundColor: C.accent, borderRadius: 3 },
});
...
const f = StyleSheet.create({
  ...
  value:    { fontSize: 14, fontWeight: '500', color: C.ink },
  placeholder: { color: C.muted },
  input:    { flex: 1, fontSize: 14, color: C.accent, fontWeight: '600', textAlign: 'right', padding: 0 },
});
...
export const AddProductScreen = ({ navigation, route }: AddProductScreenProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedId = productId ?? `${Date.now()}`;

      let finalImageUrl: string | undefined = existingUrl ?? undefined;
      if (imageUri) {
        finalImageUrl = await storageService.uploadProductImage(tenantId, savedId, imageUri);
      }
        ...
        <ScrollView
          ...
          <View style={s.imageSection}>
            <View style={s.imagePlaceholder}>
              {displayUri ? (
                ...
                <>
                  <ImageIcon size={28} color={C.muted} strokeWidth={1.5} />
            ...
            <View style={s.imageActions}>
              <TouchableOpacity style={s.imageActionBtn} onPress={pickImage} activeOpacity={0.7}>
                <Upload size={15} color={C.ink} strokeWidth={1.75} />
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}>
                <ScanBarcode size={15} color={C.muted} strokeWidth={1.75} />
                <Text style={[s.imageActionText, { color: C.muted }]}>Escanear código</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={[s.card, { gap: 0 }]}>
            <View style={s.pricesRow}>
              ...
              <View style={[s.priceCol, { borderLeftWidth: 1, borderLeftColor: C.border, paddingLeft: 16 }]}>
...
const s = StyleSheet.create({
  ...
  saveBtn:        { backgroundColor: C.accent, borderRadius: 10, paddingHorizontal: 18, paddingVertical: 10, minWidth: 80, alignItems: 'center' },
  ...
  imagePlaceholder:    { width: 110, height: 110, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, borderStyle: 'dashed', backgroundColor: C.cardBg, alignItems: 'center', justifyContent: 'center', gap: 6, overflow: 'hidden' },
  imagePlaceholderText:{ fontSize: 11, color: C.muted, fontWeight: '500' },
  ...
  imageActionBtn:      { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: C.inputBg, borderRadius: 10, borderWidth: 1.5, borderColor: C.border, paddingHorizontal: 14, paddingVertical: 11 },
  imageActionDisabled: { opacity: 0.5 },
  imageActionText:     { fontSize: 13, fontWeight: '600', color: C.ink },

  card:    { backgroundColor: C.inputBg, borderRadius: 14, borderWidth: 1.5, borderColor: C.border, paddingHorizontal: 16, paddingVertical: 4 },
  ...
  priceLabel:   { fontSize: 10, fontWeight: '700', color: C.muted, letterSpacing: 0.8 },
  priceInputRow:{ flexDirection: 'row', alignItems: 'baseline', gap: 2 },
  priceCurrency:{ fontSize: 16, fontWeight: '700', color: C.accent },
  priceInput:   { fontSize: 26, fontWeight: '800', color: C.accent, letterSpacing: -0.5, padding: 0 },

  gainRow:    { paddingBottom: 14, paddingTop: 4, borderTopWidth: 1, borderTopColor: C.border },
  gainValues: { flexDirection: 'row', alignItems: 'baseline', gap: 8, marginTop: 4 },
  gainLabel:  { fontSize: 10, fontWeight: '700', color: C.muted, letterSpacing: 0.8 },
  gainAmount: { fontSize: 20, fontWeight: '800', color: C.accent, letterSpacing: -0.5 },
  gainPct:    { fontSize: 12, color: C.muted, fontWeight: '500' },
  ...
  stockInput: { fontSize: 15, fontWeight: '700', color: C.ink, minWidth: 60, padding: 0 },
  alertHint:  { fontSize: 11, color: C.muted, marginTop: 2 },

  sliderWrap:   { paddingTop: 10, paddingBottom: 14 },
  sliderTrack:  { height: 4, backgroundColor: C.border, borderRadius: 2, position: 'relative' },
  sliderFill:   { height: 4, backgroundColor: C.accent, borderRadius: 2 },
  sliderThumb:  { position: 'absolute', top: -7, width: 18, height: 18, borderRadius: 9, backgroundColor: C.accent, borderWidth: 2.5, borderColor: '#fff', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, elevation: 3 },
  sliderLabels: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  sliderLabel:  { fontSize: 10, color: C.muted },
});
